...
$.ajax({
  ...
  data: {
    key: 'LKRBZ-ABBWJ-NOAFS-XPS7H-YWBT3-NMFB5',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  try {
    ...
    document.getElementById("welcome-info").innerHTML =
      `<b><center>🎉 欢迎信息 🎉</center>&emsp;&emsp;欢迎来自 <span style="color:var(--theme-color)">${pos}</span> 的小伙伴，当前的IP地址为： <span style="color:var(--theme-color)">${ip}</span>， ${posdesc}</b>`;
  } catch (err) {
...
function createtime1() {
  var grt = new Date("05/04/2023 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime() {
  ...
  var start = new Date("05/04/2023 00:00:00"); // 旅行者1号开始计算的时间
